...
- Translation, worksheet, and local SQLite close-lifecycle artifacts now retain rate/source lineage, effective-dated controlling ownership inputs, exact path percentages, NCI presentation, balanced elimination evidence, maker-checker run approval, exact local control-journal posting/reversal effects, and period lock/reopen evidence. Persisted ownership masters, live rates, functional-currency remeasurement, acquisition/fair-value/goodwill/equity-method policy, ownership changes, legal-book/statutory posting, PostgreSQL lifecycle parity, API/CLI/UI exposure, numbering, tax, assets, budgets, broader inventory costing/accounting beyond the bounded FIFO foundation, and statutory statements require later bounded modules.
