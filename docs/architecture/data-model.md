...
- Translation, worksheet, and local SQLite close-lifecycle artifacts now retain rate/source lineage, effective-dated controlling ownership inputs, exact path percentages, NCI presentation, balanced elimination evidence, maker-checker run approval, exact local control-journal posting/reversal effects, period lock/reopen evidence, and a pure non-posting acquisition fair-value/goodwill bridge. Persisted ownership masters, live rates, functional-currency remeasurement, full acquisition/equity-method policy, ownership-change posting, legal-book/statutory posting, PostgreSQL lifecycle parity, API/CLI/UI breadth, numbering, tax, assets, budgets, broader inventory costing/accounting beyond the bounded FIFO foundation, and statutory statements require later bounded modules.
