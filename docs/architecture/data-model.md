...
- Consolidation worksheet v1 embeds a verified translation result, approved effective-dated ownership history, exact active root paths, NCI presentation, and balanced source-bound elimination proposals. It remains migration-free and non-posting. Migration 25 separately persists verified worksheets into local SQLite consolidation close periods, runs, immutable derived journal lines, exact posting/reversal effects, and period lock/reopen events; those records are a local control-journal lifecycle only and do not mutate Finance Core legal books or a source ERP.
...
- Translation, worksheet, and local SQLite close-lifecycle artifacts now retain rate/source lineage, effective-dated controlling ownership inputs, exact path percentages, NCI presentation, balanced elimination evidence, maker-checker run approval, exact local control-journal posting/reversal effects, period lock/reopen evidence, a pure non-posting acquisition fair-value/goodwill bridge, and a source-bound non-posting acquisition PPA artifact with item-level book/fair-value adjustments. Persisted ownership masters, live rates, functional-currency remeasurement, full statutory acquisition/equity-method policy, ownership-change posting, legal-book/statutory posting, PostgreSQL PPA/lifecycle parity, API/UI breadth, numbering, tax, assets, budgets, broader inventory costing/accounting beyond the bounded FIFO foundation, and statutory statements require later bounded modules.
