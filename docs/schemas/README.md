...
- `evidence_graph.schema.json` (versioned evidence-graph nodes/edges/manifest payload with optional redaction masks and tamper-evident digest)
- `reconciliation_as_code.schema.json` (closed v1 declarative source, mapping, rule, matching, evidence, workflow, and synthetic golden-test contract; executable hooks are forbidden)
...
- `upgrade_plan.schema.json` (closed v1 approved multi-resource upgrade/rollback saga plan; no commands, scripts, secrets, or executable hooks)
- `upgrade_supported_versions.schema.json` (closed operator/resource transition matrix; `supported` is impossible while any transition is blocked)
- `ha_dr_drill_report.schema.json` (closed topology/failure/fencing/backup-integrity/RPO/RTO report with an explicit single-failure-domain maturity boundary)
- `ha_dr_repeated_drill_report.schema.json` (three-run single-host RPO/RTO distribution retaining every run and the one-failure-domain limitation)
- `offline_bundle.schema.json` (closed local-only inventory for a hash-locked `pip --no-index --require-hashes` installation input; not signature or installation evidence)
- `offline_install_drill_report.schema.json` (closed single-runtime Docker no-network installation evidence with explicit remaining Air-gap gates)
- `offline_attestation_drill_report.schema.json` (closed no-network file-attestation evidence; explicitly records OCI bundles as integrity-only when the verifier requires registry resolution)
- `airgap_identity_recovery_drill_report.schema.json` (closed single-runtime no-network local-user, encrypted-backup, isolated-restore, audit-chain, and session-invalidation evidence)
- `airgap_upgrade_rollback_drill_report.schema.json` (closed tagged application cutover and exact rollback evidence under a no-network runtime)
- `signed_airgap_install_drill_report.schema.json` (closed evidence binding the exact offline-verified signed wheel digest to the installed no-network wheel)
- `enterprise_reliability_drill_report.schema.json` (closed local dependency/capacity alert and recovery evidence)
- `otlp_http_collector_drill_report.schema.json` (closed explicit OTLP/HTTP traces, metrics, and operational-event receiver evidence)
- `postgres_reliability_drill_report.schema.json` (closed non-superuser forced-RLS PostgreSQL source-parity evidence)
- `reliability_capacity_drill_report.schema.json` (closed environment-bound local request/backlog capacity and recovery measurement)
- `otel_collector_drill_report.schema.json` (closed digest-pinned official Collector/file-backend pipeline evidence)
- `reliability_incident_drill_report.schema.json` (closed ordered acknowledgement/mitigation/recovery/closure chain with hash-only evidence references)
...
- `security_center_overview.schema.json` (closed tenant-bound count-only identity/session/integration/policy/retention/audit overview; explicitly not security assurance)
