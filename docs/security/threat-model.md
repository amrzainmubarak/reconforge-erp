...
| `retail.settlement` | Platform Security | exported POS/processor records, exact settlement decisions, input fingerprints, digest-bound reports; file ingress, SQLite/PostgreSQL persistence, API authorization, disclosure | Exact non-posting arithmetic, ambiguity, duplicate, unmatched, replay, workspace isolation, and immutable persistence controls are bounded locally/server-contract tested; provider authenticity, settlement finality, fraud, posting, write-back, Studio/accessibility, HA/DR, and production operation remain outside the slice |
| `bank.cash-reconciliation` | Platform Security | CAMT.053 statement lines, ledger exports, exact control decisions, input fingerprints, digest-bound reports; file ingress, local storage, disclosure | Exact non-posting reference/amount/date-window control, ambiguity, duplicate, unmatched, and tamper checks are bounded locally; bank authenticity, provider connectivity, posting, write-back, persistence/API/Studio, and production operation remain outside the slice |
| `manufacturing.cost-control` | Platform Security | production orders, material issues, completions, scrap events, exact cost/quantity decisions, input fingerprints, digest-bound reports; file ingress, local storage, disclosure | Exact non-posting cost/quantity, scrap, unknown-order, and tamper controls are bounded locally; statutory valuation, ERP authenticity/connectivity, inventory/WIP/GL posting, write-back, persistence/API/Studio, and production operation remain outside the slice |
