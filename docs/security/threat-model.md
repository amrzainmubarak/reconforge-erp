...
| `bank.cash-reconciliation` | Platform Security | CAMT.053 statement lines, ledger exports, exact control decisions, input fingerprints, digest-bound reports, workspace-scoped SQLite/PostgreSQL evidence, authenticated local/server APIs, synthetic Studio projection; file ingress, local storage, browser disclosure | Exact non-posting reference/amount/date-window control, ambiguity, duplicate, unmatched, tamper, immutable persistence, backup/restore, authenticated API, and disposable PostgreSQL RLS checks are evidenced; bank authenticity, provider connectivity, posting, write-back, workers, hosted deployment, HA/DR, and production operation remain outside the slice |
| `manufacturing.cost-control` | Platform Security | production orders, material issues, completions, scrap events, exact cost/quantity decisions, input fingerprints, digest-bound reports, workspace-scoped SQLite/PostgreSQL evidence, authenticated API, synthetic Studio projection; file ingress, local storage, browser disclosure | Exact non-posting cost/quantity, scrap, unknown-order, tamper, immutable persistence, tenant/workspace isolation, and bounded authenticated local/server API checks are locally evidenced; hosted PostgreSQL promotion, statutory valuation, ERP authenticity/connectivity, inventory or GL posting, write-back, and production operation remain outside the slice |
| `professional.invoice-payment` | Platform Security | exported professional invoices and client payments, exact client/amount/due-date decisions, input fingerprints, digest-bound reports; file ingress, authenticated local API, immutable workspace-scoped SQLite persistence, synthetic modern Studio projection, local storage, disclosure | Exact non-posting invoice/payment arithmetic, client binding, ambiguity, unmatched, unapplied-cash, tamper, local API authorization, and local replay/restore controls are bounded locally; revenue recognition, billing authenticity/connectivity, receivables allocation, posting, write-back, PostgreSQL server persistence, HA/DR, and production operation remain outside the slice |
| `individual.cashflow` | Platform Security | local individual/freelancer transaction and budget exports, exact category/period decisions, input fingerprints, digest-bound reports, and synthetic English/Arabic read-only Studio projection; bounded file ingress, authenticated local API, local CLI, local storage, disclosure | Exact non-posting income/expense arithmetic, category budget variance, unbudgeted activity, no-activity budget lines, tamper verification, local API authorization, and component/WCAG route coverage are bounded locally; bank authenticity/connectivity, tax/legal advice, posting, write-back, hosted persistence, HA/DR, and production operation remain outside the slice |
