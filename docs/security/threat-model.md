...
| `plugins.export` | Platform Security | built-in adapter registry, local exports, canonical projections; file ingress, local storage, build | Allowlisted built-in CSV reads share bounded preflight; signed third-party packages, malware assurance, sandboxing, credentials, network, and writeback are absent |
...
All nine modules are `experimental` in the runtime registry. The index cannot
