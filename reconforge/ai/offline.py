...
def offline_exception_explanation(exception: dict[str, object]) -> str:
    ...
    reference = str(exception.get("source_document") or exception.get("reference") or exception.get("affected_reference") or "")
    action = str(exception.get("recommended_action") or "Assign an owner, review source evidence, and document remediation.")
    return (
