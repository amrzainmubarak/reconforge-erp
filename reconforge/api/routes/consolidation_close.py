...
class DeferredTaxEvidenceAttachRequest(BaseModel):
    """Strict reference to one immutable PostgreSQL deferred-tax artifact."""

    model_config = ConfigDict(extra="forbid", strict=True)

    artifact_id: str = Field(pattern=r"^dtax-[0-9a-f]{32}$", min_length=37, max_length=37)


class PpaEvidenceAttachRequest(BaseModel):
    """Strict reference to one immutable PostgreSQL PPA artifact."""

    model_config = ConfigDict(extra="forbid", strict=True)

    artifact_id: str = Field(pattern=r"^ppa-[0-9a-f]{32}$", min_length=36, max_length=36)


class OwnershipChangeEvidenceAttachRequest(BaseModel):
    """Strict reference to one immutable PostgreSQL ownership-change artifact."""

    model_config = ConfigDict(extra="forbid", strict=True)

    artifact_id: str = Field(pattern=r"^ownchg-[0-9a-f]{32}$", min_length=39, max_length=39)
...
def attach_deferred_tax_evidence(
    run_id: str,
    request: Request,
    payload: DeferredTaxEvidenceAttachRequest,
    current_user: ConsolidationCertify,
) -> dict[str, object]:
    """Bind replay-verified, non-posting deferred-tax evidence to a close run."""

    if not server_consolidation_close_enabled(request):
        raise APIError(
            status_code=503,
            code="consolidation_deferred_tax_link_unavailable",
            message="Deferred-tax close evidence linking requires the PostgreSQL server profile.",
        )
    scope = _server_scope(request, "")
    enforce_server_scoped_permission(
        request,
    ...
    link = execute_postgres_consolidation_close(
        request,
        lambda repository, _tenant: repository.attach_deferred_tax_artifact(
            run_id,
            payload.artifact_id,
            workspace=scope.workspace_id,
            actor_label=current_user.id,
...
def attach_ppa_evidence(
    run_id: str,
    request: Request,
    payload: PpaEvidenceAttachRequest,
    current_user: ConsolidationCertify,
) -> dict[str, object]:
    """Bind replay-verified, non-posting PPA evidence to a close run."""

    if not server_consolidation_close_enabled(request):
        raise APIError(
            status_code=503,
            code="consolidation_ppa_link_unavailable",
            message="PPA close evidence linking requires the PostgreSQL server profile.",
        )
    scope = _server_scope(request, "")
    enforce_server_scoped_permission(
        request,
    ...
    link = execute_postgres_consolidation_close(
        request,
        lambda repository, _tenant: repository.attach_ppa_artifact(
            run_id,
            payload.artifact_id,
            workspace=scope.workspace_id,
            actor_label=current_user.id,
...
def attach_ownership_change_evidence(
    run_id: str,
    request: Request,
    payload: OwnershipChangeEvidenceAttachRequest,
    current_user: ConsolidationCertify,
) -> dict[str, object]:
    """Bind replay-verified, non-posting ownership-change evidence to a close run."""

    if not server_consolidation_close_enabled(request):
        raise APIError(
            status_code=503,
            code="consolidation_ownership_change_link_unavailable",
            message="Ownership-change close evidence linking requires the PostgreSQL server profile.",
        )
    scope = _server_scope(request, "")
    enforce_server_scoped_permission(
        request,
    ...
    link = execute_postgres_consolidation_close(
        request,
        lambda repository, _tenant: repository.attach_ownership_change_artifact(
            run_id,
            payload.artifact_id,
            workspace=scope.workspace_id,
            actor_label=current_user.id,
