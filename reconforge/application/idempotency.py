...
class IdempotencyReservation:
    ...
    def __post_init__(self) -> None:
        ...
        if any(not str(value).strip() for value in (self.tenant_id, self.scope, self.key, self.request_digest, self.owner_token)):
        ...
        if any(
            ord(character) < 32
            for value in (self.scope, self.key, self.owner_token)
            for character in value
        ):
            raise IdempotencyError("Idempotency identity field contains control characters.")
        if len(self.request_digest) != 64 or any(character not in "0123456789abcdef" for character in self.request_digest):
...
class IdempotencyApplicationService:
    ...
    def __init__(
        self, repository: IdempotencyRepositoryProtocol, *,
        max_request_bytes: int = 1024 * 1024, max_response_bytes: int = 1024 * 1024,
    ) -> None:
    ...
    def begin(
        self, *, tenant_id: str, scope: str, key: str, request: bytes,
        owner_token: str, created_at: str, expires_at: str, observed_at: str,
        ...
        _utc(observed_at)
        reservation = IdempotencyReservation(
            1, tenant_id, scope, key, digest_bytes(request), owner_token,
            "pending", b"", "", "", created_at, expires_at, "",
        )
        return self.repository.begin(reservation, observed_at=observed_at)

    def complete(
        self, reservation: IdempotencyReservation, *, response: bytes,
        content_type: str, completed_at: str,
    ) -> IdempotencyReservation:
        ...
        return self.repository.complete(
            reservation, response=response, response_digest=digest_bytes(response),
            content_type=content_type, completed_at=completed_at,
        )
