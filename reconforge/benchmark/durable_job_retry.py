...
records structural evidence only and explicitly does not claim provider
...
def _worker_loop(
    ...
    try:
        while not finished.is_set():
            ...
            outcome, _attempt = _process_claim(worker, claimed, profile, attempts, attempts_lock, next_now)
            with counters_lock:
...
LIMITATIONS = (
    ...
    "Retrying is immediately eligible after lease release; provider-specific exponential backoff and jitter are not claimed.",
...
def run_durable_job_retry_profile(
    database_path: Path, *, profile: DurableJobRetryProfile | None = None
