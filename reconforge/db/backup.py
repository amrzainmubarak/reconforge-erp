...
def _validate_backup(input_path: Path | str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    expected_checksum, expected_bytes, manifest_schema_version = _validate_manifest_document(manifest)
...
def _table_info_columns(connection: sqlite3.Connection, table: str) -> set[str]:
    ...
    return {str(column["name"]) for column in connection.execute(f"PRAGMA table_info({quoted_table})").fetchall()}
            ...
            for table in BACKUP_TABLES:
                ...
                if table == "inventory_valuation_reversals" and isinstance(rows, list):
                    ...
                    for row in rows:
                        ...
                        status = str(row.get("status", "Draft"))
                        if status not in {"Draft", "Approved", "Cancelled"}:
                            raise DBBridgeError("Backup contains an invalid inventory-valuation reversal status.")
