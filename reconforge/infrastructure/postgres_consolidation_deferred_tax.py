"""Tenant-RLS PostgreSQL persistence for non-posting deferred-tax evidence."""
...
POSTGRES_CONSOLIDATION_DEFERRED_TAX_SCHEMA_SQL = r"""
CREATE TABLE IF NOT EXISTS reconforge.consolidation_deferred_tax_artifacts (
    tenant_id TEXT NOT NULL,
    id TEXT NOT NULL CHECK (id ~ '^dtax-[0-9a-f]{32}$'),
    ...
    CONSTRAINT consolidation_deferred_tax_actor_separation CHECK (prepared_by <> approved_by),
...
CREATE INDEX IF NOT EXISTS consolidation_deferred_tax_scope_idx
    ON reconforge.consolidation_deferred_tax_artifacts
        (tenant_id, acquisition_id, period_id, created_at, id);
ALTER TABLE reconforge.consolidation_deferred_tax_artifacts ENABLE ROW LEVEL SECURITY;
ALTER TABLE reconforge.consolidation_deferred_tax_artifacts FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS tenant_scope ON reconforge.consolidation_deferred_tax_artifacts;
CREATE POLICY tenant_scope ON reconforge.consolidation_deferred_tax_artifacts
    USING (tenant_id = current_setting('app.tenant_id', true))
    WITH CHECK (tenant_id = current_setting('app.tenant_id', true));
CREATE OR REPLACE FUNCTION reconforge.guard_consolidation_deferred_tax_artifact()
RETURNS TRIGGER LANGUAGE plpgsql AS $reconforge$
BEGIN
    IF TG_OP = 'UPDATE' THEN
        RAISE EXCEPTION 'consolidation deferred-tax artifacts are immutable' USING ERRCODE='check_violation';
    END IF;
    IF TG_OP = 'DELETE' THEN
        RAISE EXCEPTION 'consolidation deferred-tax artifacts cannot be deleted' USING ERRCODE='check_violation';
    END IF;
    RETURN NEW;
END
$reconforge$;
DROP TRIGGER IF EXISTS consolidation_deferred_tax_artifact_guard
    ON reconforge.consolidation_deferred_tax_artifacts;
CREATE TRIGGER consolidation_deferred_tax_artifact_guard
BEFORE UPDATE OR DELETE ON reconforge.consolidation_deferred_tax_artifacts
FOR EACH ROW EXECUTE FUNCTION reconforge.guard_consolidation_deferred_tax_artifact();
"""


class PostgresConsolidationDeferredTaxError(RuntimeError):
    """Safe PostgreSQL deferred-tax persistence failure."""
...
def _payload(value: object, field: str) -> dict[str, object]:
    try:
        ...
        raise PostgresConsolidationDeferredTaxError(f"Persisted deferred-tax {field} is invalid.") from exc
    if not isinstance(decoded, dict):
        raise PostgresConsolidationDeferredTaxError(f"Persisted deferred-tax {field} is invalid.")
    return dict(decoded)
...
class PostgresConsolidationDeferredTaxRepository:
    ...
    def _actor(value: str) -> str:
        ...
        if not actor:
            raise PlatformError("Deferred-tax actor is required.")
        return actor
    ...
    def _verify_result(
        ...
        if not isinstance(request, AcquisitionDeferredTaxBridgeRequest):
            raise PostgresConsolidationDeferredTaxError("A typed deferred-tax request is required.")
        if not isinstance(result, AcquisitionDeferredTaxBridgeResult):
            raise PostgresConsolidationDeferredTaxError("A typed deferred-tax result is required.")
        try:
            expected = prepare_acquisition_deferred_tax_bridge(request)
            payload = result.to_dict()
            verify_acquisition_deferred_tax_bridge_payload(payload)
        except (ConsolidationError, TypeError, ValueError) as exc:
            raise PostgresConsolidationDeferredTaxError(
                "Deferred-tax evidence failed deterministic verification."
            ) from exc
        if result.request_digest != request.digest or result.result_digest != expected.result_digest:
            raise PostgresConsolidationDeferredTaxError("Deferred-tax request/result digest lineage is invalid.")
        if payload.get("posted") is not False:
            raise PostgresConsolidationDeferredTaxError(
                "PostgreSQL deferred-tax persistence cannot store posted artifacts."
            )
        return payload
    ...
    def _decode_row(cls, row: Any) -> dict[str, Any]:
        ...
        if _json_digest(request_payload) != request_digest:
            raise PostgresConsolidationDeferredTaxError("Persisted deferred-tax request digest mismatch.")
        result_payload = _payload(_row_value(row, "result_payload", 10), "result payload")
        try:
            verify_acquisition_deferred_tax_bridge_payload(result_payload)
        except (ConsolidationError, TypeError, ValueError) as exc:
            raise PostgresConsolidationDeferredTaxError(
                "Persisted deferred-tax evidence failed replay verification."
            ) from exc
        if result_payload.get("posted") is not False:
            raise PostgresConsolidationDeferredTaxError("Persisted deferred-tax evidence is unexpectedly posted.")
        if result_payload.get("request_digest") != request_digest:
            raise PostgresConsolidationDeferredTaxError("Persisted deferred-tax lineage is inconsistent.")
        result_digest = str(_row_value(row, "result_digest", 8))
        if result_payload.get("result_digest") != result_digest:
            raise PostgresConsolidationDeferredTaxError("Persisted deferred-tax result digest mismatch.")
        return {
            ...
            "acquisition_date": str(_row_value(row, "acquisition_date", 5)),
    ...
    def persist(
        ...
        *,
        ...
        if actor != request.prepared_by:
            raise PlatformError("The authenticated deferred-tax preparer must match the request preparer.")
        payload = self._verify_result(request, result)
        identifier = artifact_id or self._artifact_id(request)
        if not isinstance(identifier, str) or re.fullmatch(r"dtax-[0-9a-f]{32}", identifier) is None:
            raise PlatformError("Deferred-tax artifact ID is invalid.")
        try:
            with self.connection.transaction():
                ...
                if duplicate is not None:
                    ...
                    if stored["request_digest"] != request.digest:
                        raise PlatformError("Deferred-tax result digest conflicts with immutable evidence.")
                    return stored
                ...
                PostgresAuditEventRepository(self.connection, self.tenant_id).append(
                    actor_label=actor,
                    actor_user_id=actor,
                    object_type="consolidation_deferred_tax_artifact",
                    object_id=identifier,
                    action="consolidation_deferred_tax_artifact_created",
        ...
        except (PlatformError, PostgresConsolidationDeferredTaxError):
            raise
        except Exception as exc:
            raise PostgresConsolidationDeferredTaxError("PostgreSQL deferred-tax persistence failed.") from exc

    def get(self, artifact_id: str, *, actor_label: str = "local-cli") -> dict[str, Any]:
        self._actor(actor_label)
        if not isinstance(artifact_id, str) or re.fullmatch(r"dtax-[0-9a-f]{32}", artifact_id) is None:
            raise PlatformError("Deferred-tax artifact ID is invalid.")
        try:
            with self.connection.transaction():
                self._scope()
            ...
            raise PostgresConsolidationDeferredTaxError("PostgreSQL deferred-tax read failed.") from exc
