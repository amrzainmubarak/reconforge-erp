"""Workspace-scoped immutable persistence for bank-statement control evidence."""
...
class BankStatementPersistenceError(ValueError):
...
def _artifact_payload(run: BankStatementControlRun) -> dict[str, object]:
    payload = run.to_dict()
...
class SQLiteBankStatementRepository:
    """Persist one replay-verifiable report without posting or provider I/O."""

    def __init__(self, connection: sqlite3.Connection) -> None:
        ensure_platform_schema(connection)
        self.connection = connection
        exists = self.connection.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='bank_statement_control_runs'",
        ).fetchone()
        if exists is None:
            raise BankStatementPersistenceError("bank statement persistence migration is required")
    ...
    def _actor(self, actor_label: str) -> str:
        try:
            ...
            raise BankStatementPersistenceError("bank statement actor is not authorized") from exc
    ...
    def _validate_payload(payload: Mapping[str, object]) -> None:
        if payload.get("artifact_type") != _ARTIFACT_TYPE:
            raise BankStatementPersistenceError("bank statement artifact type is invalid")
        ...
        if not isinstance(digest, str) or digest != _artifact_digest(without_digest):
            raise BankStatementPersistenceError("bank statement artifact digest mismatch")
        try:
            verify_bank_statement_payload(dict(payload))
        except (BankStatementControlError, KeyError, TypeError, ValueError) as exc:
            raise BankStatementPersistenceError("bank statement replay verification failed") from exc

    def put(
        self,
        run: BankStatementControlRun,
        *,
        workspace: str = "default",
        actor_label: str = "local-cli",
    ) -> dict[str, Any]:
        if not isinstance(run, BankStatementControlRun):
            raise BankStatementPersistenceError("bank statement run is invalid")
    ...
    def put_payload(
        ...
        *,
        ...
        if not isinstance(payload, Mapping):
            raise BankStatementPersistenceError("bank statement report is invalid")
        payload_value = dict(payload)
        try:
            encoded = encode_sqlite_bank_statement_control(payload_value)
        except PersistedJsonError as exc:
            raise BankStatementPersistenceError("bank statement payload exceeds persistence bounds") from exc
        ...
        if not all(isinstance(value, str) for value in (decision_digest, algorithm_version, artifact_digest)):
            raise BankStatementPersistenceError("bank statement report identity is invalid")
        if not isinstance(status_counts, Mapping):
            raise BankStatementPersistenceError("bank statement report status counts are invalid")
        actor = self._actor(actor_label)
        workspace_id = ensure_workspace(self.connection, workspace)
        run_id = platform_id("BNK", workspace_id, decision_digest)
        existing = self.connection.execute(
            "SELECT * FROM bank_statement_control_runs WHERE id=?",
            (run_id,),
        ).fetchone()
        if existing is not None:
            if str(existing["artifact_digest"]) != artifact_digest:
                raise BankStatementPersistenceError("bank statement id conflicts with a different artifact")
        ...
        try:
            ...
            commit_audited(
                self.connection,
                actor_label=actor,
                object_type="bank_statement_control_run",
                object_id=run_id,
                action="bank_statement_control_persisted",
                metadata={
            ...
            if raced is not None:
                ...
                raise BankStatementPersistenceError("bank statement id conflicts with a different artifact") from exc
            raise BankStatementPersistenceError("bank statement persistence failed") from exc
    ...
    def get(self, *, decision_digest: str, workspace: str = "default") -> dict[str, Any] | None:
        workspace_id = ensure_workspace(self.connection, workspace)
        row = self.connection.execute(
            "SELECT * FROM bank_statement_control_runs WHERE workspace_id=? AND decision_digest=?",
    ...
    def list(
        ...
        *,
        ...
        if isinstance(limit, bool) or not 1 <= limit <= 500:
            raise BankStatementPersistenceError("bank statement list limit is invalid")
        if isinstance(offset, bool) or offset < 0:
            raise BankStatementPersistenceError("bank statement list offset is invalid")
        workspace_id = ensure_workspace(self.connection, workspace)
        rows = self.connection.execute(
            "SELECT * FROM bank_statement_control_runs WHERE workspace_id=? ORDER BY created_at,id LIMIT ? OFFSET ?",
    ...
    def _row_to_public(self, row: sqlite3.Row | None) -> dict[str, Any]:
        if row is None:
            raise BankStatementPersistenceError("bank statement persistence row is missing")
        try:
            document = decode_sqlite_bank_statement_control(str(row["payload_json"]))
        except PersistedJsonError as exc:
            raise BankStatementPersistenceError("bank statement persisted JSON is invalid") from exc
        payload = document.payload
        self._validate_payload(payload)
        if str(row["artifact_digest"]) != str(payload.get("artifact_digest")):
            raise BankStatementPersistenceError("bank statement persisted digest mismatch")
        if str(row["decision_digest"]) != str(payload.get("decision_digest")):
            raise BankStatementPersistenceError("bank statement persisted decision mismatch")
        if str(row["algorithm_version"]) != str(payload.get("algorithm_version")):
            raise BankStatementPersistenceError("bank statement persisted algorithm mismatch")
        ...
        if str(row["status_counts_json"]) != expected_status_counts:
            raise BankStatementPersistenceError("bank statement persisted status mismatch")
        return {
            "id": str(row["id"]),
...
__all__ = ["BankStatementPersistenceError", "SQLiteBankStatementRepository"]
