"""SQLite persistence adapter for close management."""
...
class SQLiteCloseManagementRepository:
    """Own local close persistence, invariants, and transaction effects."""
    ...
    def task_dependency(
        ...
        *,
        ...
        if task_id == depends_on_task_id or self._dependency_reaches(depends_on_task_id, task_id):
    ...
    def reopen_period(self, period_id: str, *, reason: str, actor_label: str = "local-cli") -> dict[str, Any]:
        ...
        try:
            ...
            self.connection.rollback()
            raise PlatformError("Unable to reopen close period.") from exc
        return self.get_period(period_id)
    ...
    def _dependency_reaches(self, start_task_id: str, target_task_id: str) -> bool:
        row = self.connection.execute(
            """
            WITH RECURSIVE dependency_path(task_id) AS (
                SELECT depends_on_task_id FROM close_task_dependencies WHERE task_id = ?
                UNION
                SELECT dependency.depends_on_task_id
                FROM close_task_dependencies dependency
                JOIN dependency_path path ON dependency.task_id = path.task_id
            )
            SELECT 1 FROM dependency_path WHERE task_id = ? LIMIT 1
            """,
