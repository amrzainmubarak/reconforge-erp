"""Persistence-independent deterministic governed matching engine."""
...
def _resolve_currency_precision(
    resolver: CurrencyPrecisionResolver,
    cache: dict[str, tuple[int | None, str | None]],
    raw_code: str,
) -> tuple[str, int | None]:
    """Resolve minor-unit precision through a backend-neutral lookup port."""
    ...
    precision, issue = resolver(raw_code)
    cache[raw_code] = (precision, issue)
    if issue is not None:
        raise InvalidAmountError(issue)
    return raw_code, precision
...
class DeterministicMatchingEngine:
    """Persistence-independent deterministic record matching engine."""

    def __init__(
        self,
        currency_precision_resolver: CurrencyPrecisionResolver,
        *,
        max_candidates_per_left_record: int = MAX_CANDIDATES_PER_LEFT_RECORD,
        max_total_candidate_evaluations: int = MAX_TOTAL_CANDIDATE_EVALUATIONS,
    ) -> None:
        self.currency_precision_resolver = currency_precision_resolver
        self.max_candidates_per_left_record = max_candidates_per_left_record
        self.max_total_candidate_evaluations = max_total_candidate_evaluations
        ...
        *,
        ...
        for left_index, budget in sorted(candidate_budget_failures.items()):
            ...
            evidence: dict[str, object] = {
                ...
                "limit": budget["limit"],
                "max_candidates_per_left_record": self.max_candidates_per_left_record,
                "max_total_candidate_evaluations": self.max_total_candidate_evaluations,
