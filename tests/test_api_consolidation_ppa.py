...
from reconforge.api.routes import consolidation_ppa as routes
from reconforge.auth.service import LocalAuthService
from reconforge.db import connect, run_migrations
from tests.test_consolidation_ppa import _request
...
def test_ppa_api_is_server_profile_only_and_authenticated(tmp_path: Path) -> None:
    ...
    unavailable = client.post("/api/v1/consolidation-ppa", headers=headers, json=_body())
    assert unavailable.status_code == 503
    assert unavailable.json()["error"]["code"] == "consolidation_ppa_unavailable"


def test_ppa_api_rebuilds_typed_request_and_uses_authenticated_actor(
    ...
    monkeypatch.setattr(routes, "execute_postgres_ppa", execute)
    ...
    loaded = client.get(
        "/api/v1/consolidation-ppa/ppa-" + "a" * 32,
        headers=headers,
    )
...
def test_ppa_server_routes_re_evaluate_tenant_policy_without_workspace(
    ...
    assert captured == [(frozenset({"finance_core.manage"}), "tenant-a", None, Decimal("200.00"))]
