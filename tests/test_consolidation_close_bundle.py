...
def test_close_bundle_binds_ownership_change_evidence_digests() -> None:
    detail = {
        "id": "RUN-OWNCHG-1",
        "period_id": "PERIOD-1",
        "workspace_id": "WORKSPACE-1",
        ...
        "effects": [],
        "journal_digest": "e" * 64,
        "ownership_change_evidence": [{"artifact_result_digest": "f" * 64}],
    }

    bundle = build_consolidation_close_bundle(detail)
    assert bundle.ownership_change_artifact_digests == ("f" * 64,)
    assert verify_consolidation_close_bundle_payload(bundle.to_dict()).to_dict() == bundle.to_dict()
