...
def _record(
    record_id: str, amount: str, *, currency: str = "USD", day: int = 10, partition: str = "AR"
) -> GroupedRecord:
    return GroupedRecord(record_id, Decimal(amount), currency, date(2026, 1, day), partition)
...
def test_grouped_match_is_permutation_invariant_and_uses_stable_tie_break() -> None:
    ...
    assert baseline.right_record_ids == ("R1", "R2")
    assert "stable record identities broke a tie" in baseline.explanation
...
def test_grouped_match_rejects_binary_or_non_finite_money_and_invalid_policy() -> None:
    with pytest.raises(GroupedMatchingError):
        GroupedRecord("L1", 1.5, "USD", date(2026, 1, 1), "AR")  # type: ignore[arg-type]
    with pytest.raises(GroupedMatchingError):
        _record("L1", "NaN")
    with pytest.raises(GroupedMatchingError):
