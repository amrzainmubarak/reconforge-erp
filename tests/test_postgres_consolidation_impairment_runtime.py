...
def test_live_postgres_impairment_persistence_is_replayable_immutable_and_tenant_isolated() -> None:
    ...
    suffix = uuid4().hex[:10]
    tenant_a = f"imp_a_{suffix}"
    tenant_b = f"imp_b_{suffix}"
    user_one = f"imp_preparer_{suffix}"
    user_two = f"imp_reviewer_{suffix}"
    ...
    try:
        with admin.transaction():
            ...
            admin.execute(
                f"GRANT SELECT, INSERT ON TABLE reconforge.consolidation_impairment_artifacts, "
        ...
        with PostgresTenantBoundary(app_factory).transaction(tenant_a) as connection:
            service = ConsolidationImpairmentApplicationService(
        ...
        with pytest.raises(PostgresConsolidationImpairmentError, match="not found"), PostgresTenantBoundary(
            app_factory
        ).transaction(tenant_b) as connection:
            ConsolidationImpairmentApplicationService(
                PostgresConsolidationImpairmentRepository(connection, tenant_b)
            ).get(str(created["id"]), actor_label=user_one)

        with pytest.raises(Exception, match="consolidation impairment artifacts are immutable"), admin.transaction():
            admin.execute(
                "UPDATE reconforge.consolidation_impairment_artifacts SET entity_code=%s WHERE tenant_id=%s AND id=%s",
                ("tampered", tenant_a, created["id"]),
            )
        with pytest.raises(Exception, match="consolidation impairment artifacts cannot be deleted"), admin.transaction():
            admin.execute(
                "DELETE FROM reconforge.consolidation_impairment_artifacts WHERE tenant_id=%s AND id=%s",
                (tenant_a, created["id"]),
            )
    finally:
        with admin.transaction():
            admin.execute(
                "ALTER TABLE reconforge.consolidation_impairment_artifacts DISABLE TRIGGER consolidation_impairment_artifact_guard"
            )
            admin.execute(
                "DELETE FROM reconforge.consolidation_impairment_artifacts WHERE tenant_id IN (%s,%s)",
                (tenant_a, tenant_b),
            )
            admin.execute(
                "ALTER TABLE reconforge.consolidation_impairment_artifacts ENABLE TRIGGER consolidation_impairment_artifact_guard"
            )
